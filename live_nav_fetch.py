...
KEY_SCHEMES = {
    125497: "HDFC Top 100 Fund - Direct Plan",
...
def fetch_mfapi_nav(scheme_code, scheme_name=None):
...
def fetch_all(out_dir="data/raw"):
    ...
    for code, name in KEY_SCHEMES.items():
        df, meta = fetch_mfapi_nav(code, name)
        df.to_csv(os.path.join(out_dir, f"live_nav_{code}.csv"), index=False)
        frames.append(df)
        print(f"Fetched {name} ({code}): {len(df)} rows")
